 ...
 *   label = @Translation("Upload files as media items"),
...
class MediaFileUpload extends FileUpload {
  ...
  protected function prepareEntities(array $form, FormStateInterface $form_state) {
    ...
    foreach ($files as $file) {
      ...
      $media_file = $this->entityTypeManager->getStorage('media')->create([
  ...
  public function submit(array &$element, array &$form, FormStateInterface $form_state) {
    if (!empty($form_state->getTriggeringElement()['#eb_widget_main_submit'])) {
      $media_files = $this->prepareEntities($form, $form_state);
      array_walk(
        $media_files,
      ...
      $this->selectEntities($media_files, $form_state);
